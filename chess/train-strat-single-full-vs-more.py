...
df["labels"] = df["labels"].apply(lambda x: "full" if len(x) == 32 else x)
singles = df[df["count"] == 1]
full = df[df['count'] == 32]
xdf = pd.concat((singles, full), axis=0)
singles_train, singles_test = map(
    lambda x: x.index,
    train_test_split(xdf, stratify=xdf["labels"], train_size=0.6),
)
...
torch.save(model, "data/models/strat_single_full.pt")
...
metrics.to_csv(f"data/output/chess_iou_strat_single_full_vs_more_{datetime.now().isoformat()}.csv")
print(metrics.mean())
