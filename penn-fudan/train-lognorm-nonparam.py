...
# Stratification: Nonparametric, by average size of mask
X = []
for image, ann in dataset_test:
    h, w = image.shape[1:]
    fc = ann["masks"].numpy().sum(-1).sum(-1) / (h * w)
    X.append({"id": ann["image_id"].item(), "X": fc.mean()})
df = pd.DataFrame.from_records(X)
df.set_index("id", inplace=True, verify_integrity=True)
edges = np.arange(0.02, 0.22, 0.02)
binlabels = pd.cut(
    df["X"], edges, right=True, labels=list(string.ascii_uppercase[: len(edges) - 1])
)
binlabels[binlabels == "I"] = "H"
df['binlabels'] = binlabels

train, test = train_test_split(df, stratify=df['binlabels'], train_size=100)
# End stratification
